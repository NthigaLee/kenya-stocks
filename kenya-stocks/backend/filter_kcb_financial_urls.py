...
def get_kcb_financial_statement_links():
    ...
    try:
        ...
        pdf_links = set()  # Use a set to automatically deduplicate
...
def filter_financial_statements(urls):
    """Filter URLs to only include financial statements."""
    financial_keywords = ['financial', 'results', 'statement', 'earnings', 'income', 'balance', 'cashflow']
    financial_urls = []
    
    for url in urls:
        filename = url.split('/')[-1].lower()
        if any(keyword in filename for keyword in financial_keywords):
            financial_urls.append(url)
    
    return financial_urls

def main():
    print("Fetching KCB financial statement links...")
    all_links = get_kcb_financial_statement_links()
    
    print(f"Found {len(all_links)} total PDF links")
    
    # Filter to only financial statements
    financial_links = filter_financial_statements(all_links)
    
    print(f"Found {len(financial_links)} financial statement links:")
