"""
fetch_prices.py — Fetch latest NSE stock prices and market index data.
...
Outputs:
  - data/nse/prices.json      (per-stock prices)
  - frontend/market.json       (NSE indices + top movers for dashboard)
"""
...
MARKET_FILE = FRONTEND_DIR / "market.json"
...
def fetch_indices() -> dict:
    """Fetch NSE 20 and NSE All Share index values via yfinance."""
    indices = {}
    try:
        import yfinance as yf
    except ImportError:
        print("yfinance not installed, skipping indices.", file=sys.stderr)
        return indices
    ...
    for key, symbol in index_symbols.items():
        try:
            ...
            prev_val = float(hist["Close"].iloc[-2]) if len(hist) >= 2 else last_val
            change_pct = ((last_val - prev_val) / prev_val * 100) if prev_val else 0
            indices[key] = {
                "value": round(last_val, 2),
                "change_pct": round(change_pct, 2),
            }
            print(f"  ✓ {key}: {last_val:.2f} ({change_pct:+.2f}%)")
        except Exception as e:
...
def generate_market_json(prices: dict, indices: dict):
    """Generate frontend/market.json with index data and top movers."""
    # Find top gainer and top loser
    movers = [(t, p) for t, p in prices.items() if p.get("change_pct") is not None]
    ...
    if movers:
        g = movers[0]
        top_gainer = {"ticker": g[0], "price": g[1]["price"], "change_pct": g[1]["change_pct"]}
        l = movers[-1]
        top_loser = {"ticker": l[0], "price": l[1]["price"], "change_pct": l[1]["change_pct"]}

    market = {
        "nse20": indices.get("nse20", {"value": None, "change_pct": None}),
        "nseAllShare": indices.get("nseAllShare", {"value": None, "change_pct": None}),
        "topGainer": top_gainer,
        "topLoser": top_loser,
        "stocksTracked": len(prices),
    ...
    print(f"✓ Written market.json to {MARKET_FILE}")
...
def main():
    ...
    print("\n=== Fetching NSE Indices ===")
    indices = fetch_indices()

    # Write per-stock prices
    ...
    # Generate market.json for frontend
    generate_market_json(prices, indices)
